...
- Containment limitation: Node.js exposes no directory-handle-relative, no-follow creation primitive (no mkdirat/openat), so a *concurrent local attacker* who replaces a parent directory in the final check-to-create window can redirect a directory creation outside the root. Kestrel detects the escape canonically, classifies it as DM_UNSAFE_PATH, and leaves the empty artifact for the operator; it does not claim race-free containment against that concurrent-local-attacker model.
