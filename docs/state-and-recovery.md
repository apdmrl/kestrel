...
- Exact recovery boundaries are proven by a FIFO barrier that is a no-op in production and activates only under `NODE_ENV=test` with dedicated env vars. Seven built-CLI cases each crash immediately after one of the seven persisted preparation checkpoints, and three built-CLI cases each crash immediately after a real accept transaction reaches `PREPARED`, `STATE_WRITTEN`, or `EVENT_APPENDED`; each is recovered via the product `break-lock`, resumed, and asserted to converge with at-most-once side effects (see `test/e2e/workflows.test.ts`). The matrix passed three consecutive runs.
